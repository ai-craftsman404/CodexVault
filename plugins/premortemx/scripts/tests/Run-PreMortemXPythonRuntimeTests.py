...
def execute_workflow_case(
    ...
    *,
    ...
    try:
        ...
        journey = json.loads((run_path / "journey-view.json").read_text(encoding="utf-8"))
        journey_html = (run_path / "journey-view.html").read_text(encoding="utf-8")
        ...
        assert_true(journey["gateCard"]["finalRecommendation"] == expected_decision, f"Workflow case `{project_slug}` journey gate card should mirror the final decision.")
        assert_true(journey["gateCard"]["approvalStatus"] == expected_approval_status, f"Workflow case `{project_slug}` journey gate card should mirror approval status.")
        assert_true(journey["gateCard"]["gateStatus"] == expected_gate_status, f"Workflow case `{project_slug}` journey gate card should mirror gate status.")
        assert_true(journey["gateCard"]["allowedNextStage"] == expected_allowed_next_stage, f"Workflow case `{project_slug}` journey gate card should mirror allowed next stage.")
        assert_true(len(journey["journey"]["nodes"]) >= 6, f"Workflow case `{project_slug}` journey should include stage nodes.")
        assert_true("<svg" in journey_html, f"Workflow case `{project_slug}` journey HTML should render inline SVG.")
        assert_true(expected_current_action in journey_html, f"Workflow case `{project_slug}` journey HTML should show the current action.")
...
def main() -> None:
    ...
    try:
        ...
        start_here = (run_path / "start-here.md").read_text(encoding="utf-8")
        journey_view = json.loads((run_path / "journey-view.json").read_text(encoding="utf-8"))
        journey_html = (run_path / "journey-view.html").read_text(encoding="utf-8")
        ...
        assert_true("[Journey view](journey-view.html)" in start_here, "Python start-here should link to the journey view.")
        ...
        assert_true(record_payload["artifacts"]["journeyView"] == "journey-view.html", "Python run record should register the journey HTML artifact.")
        assert_true(record_payload["artifacts"]["journeyData"] == "journey-view.json", "Python run record should register the journey JSON artifact.")
        ...
        assert_true(journey_view["schemaVersion"] == "v6a-1", "Python journey view should use the v6a schema version.")
        assert_true(journey_view["mode"] == "release-risk-gating", "Python journey view should bind to the release-risk mode by default.")
        assert_true("journey" in journey_view and "nodes" in journey_view["journey"], "Python journey view should include a derived node graph.")
        assert_true(journey_view["gateCard"]["finalRecommendation"] == record_payload["decision"], "Python journey view should mirror the run recommendation.")
        assert_true(journey_view["isCompact"] is False, "Python initial Warn run should not collapse to compact journey mode.")
        assert_true("<svg" in journey_html, "Python journey HTML should render inline SVG.")
        assert_true("Gate Card" in journey_html, "Python journey HTML should render the gate card section.")
        assert_true("This visual surface is derived from the governed run artifacts" in journey_html, "Python journey HTML should explain the governed surface.")
        tests_run += 1
        ...
        journey_after = json.loads((run_path / "journey-view.json").read_text(encoding="utf-8"))
        journey_html_after = (run_path / "journey-view.html").read_text(encoding="utf-8")
        ...
        assert_true(journey_after["gateCard"]["finalRecommendation"] == "Block", "Python journey view should refresh to the adjudicated Block decision.")
        assert_true(journey_after["gateCard"]["allowedNextStage"] is None, "Python journey view should keep next-stage progression blocked after deliberation.")
        assert_true(journey_after["gateCard"]["resumeGuidance"] == record["runtimeState"]["resumeGuidance"], "Python journey view should refresh resume guidance after deliberation.")
        assert_true(journey_after["isCompact"] is False, "Python blocked run should not collapse to compact journey mode.")
        assert_true("Human escalation is required before the workflow can proceed." in journey_html_after, "Python journey HTML should refresh the post-deliberation confirmation reason.")
        tests_run += 1
