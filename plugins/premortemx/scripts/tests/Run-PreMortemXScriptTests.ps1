...
function Test-RunInitialization {
    ...
    $summaryShort = Get-Content -LiteralPath (Join-Path $created.RunPath "summary-short.md") -Raw
    $summaryStandard = Get-Content -LiteralPath (Join-Path $created.RunPath "summary-standard.md") -Raw
    $summaryExec = Get-Content -LiteralPath (Join-Path $created.RunPath "summary-exec.md") -Raw
    $riskRegister = Get-Content -LiteralPath (Join-Path $created.RunPath "risk-register.md") -Raw
    $analysis = Get-Content -LiteralPath (Join-Path $created.RunPath "analysis.json") -Raw | ConvertFrom-Json
    $evidenceIndex = Get-Content -LiteralPath (Join-Path $created.RunPath "evidence-index.md") -Raw
    $deliberation = Get-Content -LiteralPath (Join-Path $created.RunPath "deliberation.json") -Raw | ConvertFrom-Json

    Assert-True ($summaryShort -match "## Assessment Scope") "Short summary should include assessment scope."
    Assert-True ($summaryStandard -match "## Assessment Scope And Decision Window") "Standard summary should include scope and decision window."
    Assert-True ($summaryStandard -match "## Decision Owner And Required Action") "Standard summary should include decision owner section."
    Assert-True ($summaryExec -match "## Decision Required") "Executive summary should include decision required section."
    Assert-True ($summaryExec -match "## Proceed Conditions") "Executive summary should include proceed conditions."
    Assert-True ($riskRegister -match "Likelihood:") "Risk register should scaffold likelihood."
    Assert-True ($riskRegister -match "Risk owner:") "Risk register should scaffold risk ownership."
    Assert-True ($evidenceIndex -match "supports finding IDs") "Evidence index should scaffold traceability fields."
    Assert-True ($analysis.decisionContext -ne $null) "Analysis scaffold should include decision context."
    Assert-True (@($analysis.findings).Count -ge 1) "Analysis scaffold should include finding placeholders."
    Assert-True ($deliberation.overrideModel.overrideTestSummary -eq "Pending override evaluation.") "Deliberation scaffold should include override test summary."
    $script:testsRun++
...
'@ | Set-Content -LiteralPath $specialistInputPath
    ...
    Assert-True ([string]::IsNullOrWhiteSpace($record.deliberation.overrideModel.overrideTestSummary) -eq $false) "Deliberation should record override test summary."
    Assert-True ([string]::IsNullOrWhiteSpace($record.deliberation.adjudication.ruleTriggered) -eq $false) "Deliberation should record rule trigger."
    Assert-True (@($record.deliberation.adjudication.decisionPath).Count -ge 1) "Deliberation should record decision path."
    Assert-True ($null -ne $record.deliberation.adjudication.humanEscalationConsidered) "Deliberation should record whether human escalation was considered."
    $script:testsRun++
}
