...
- started `v6a` implementation with a derived per-run risk-journey surface
- added generated `journey-view.json` and `journey-view.html` artifacts to the Python runtime path
- rendered the first `v6a` visual layer as `HTML + inline SVG` over canonical governed run artifacts
- linked the journey view from `start-here.md` and kept it refreshed after deliberation
- extended the Python runtime suite to validate journey schema, gate-card state, HTML rendering, and workflow-case refresh behavior
