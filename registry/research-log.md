...
| 2026-05-21 | VaultGPT official Codex docs refresh | OpenAI Developers: Codex build plugins, skills, Chrome extension, in-app browser, agent approvals/security, subagents | Current guidance supports VaultGPT as a skill-plus-scripts plugin. Chrome extension is the right Codex surface for signed-in ChatGPT selected-chat capture; in-app browser is not suitable for signed-in ChatGPT because it lacks auth/profile/cookies/extensions. Browser content must be treated as untrusted, and capture/export/bulk actions need explicit approvals, privacy review, and audit logging. Subagents are appropriate for selective governed review, not every small operation. | Recheck before MVP design freeze, before public release, or if Codex Chrome/plugin capabilities change |
