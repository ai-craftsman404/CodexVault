...
def _derive_journey_view(record: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, blocker in enumerate(record["runtimeState"]["proceedBlockers"], start=1):
        ...
        add_node(
            ...
            f"Blocker {index}",
            "blocked",
            "approval" if "approval" in blocker.lower() or "confirmation" in blocker.lower() else "boundary",
            blocker,
            owner=_decision_owner(record),
            next_action=_next_action_summary(record),
            resume_stage=record["workflowState"]["nextStage"],
        ...
        if record["runtimeState"]["resumeGuidance"]:
            reentry_node_id = f"reentry-{index}"
            add_node(
                reentry_node_id,
                blocker_node_id,
                "reentry",
                ...
                "Resume path",
                "resume",
                "reentry" if record["workflowState"]["activeExceptionRule"] else "remediation",
    ...
    gate_card = {
        ...
        "allowedNextStage": record["workflowState"]["allowedNextStage"],
        "decisionOwner": _decision_owner(record),
        "nextOwner": _decision_owner(record),
        "resumeGuidance": record["runtimeState"]["resumeGuidance"],
        "boundaryStatus": record["runtimeState"]["boundaryStatus"],
