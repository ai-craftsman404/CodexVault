...
$ruleTriggered = if ($overrideReason -match "^Severity-and-policy") {
    "severity-and-policy"
}
elseif ($overrideReason -match "^Evidence-gated") {
    "evidence-gated"
}
else {
    "consensus"
...
$decisionPath = @(
    "Consensus began at $consensusDecision with disagreement level $disagreementLevel.",
    "Overall confidence resolved to $overallConfidence on the internal 1-5 scale.",
    "Minimum evidence completeness resolved to $minimumEvidenceCompleteness and minimum policy fit resolved to $minimumPolicyFit."
)

if ($overrideApplied -and $overrideReason) {
    $decisionPath += "Override path: $overrideReason"
}
else {
    $decisionPath += "No override applied; the adjudicated outcome remained aligned with the consensus decision."
}

$uncertaintyDrivers = @()
if ($minimumEvidenceCompleteness -le 2) {
    $uncertaintyDrivers += "evidence completeness remained low"
}
if ($disagreementLevel -ge 4) {
    $uncertaintyDrivers += "specialist disagreement remained high"
}
if (($record.permissionEscalationsRequested | Measure-Object).Count -gt ($record.permissionEscalationsApproved | Measure-Object).Count) {
    $uncertaintyDrivers += "permission broadening was unresolved"
}

$residualRiskAfterMitigation = switch ($finalDecision) {
    "Pass" { "Low" }
    "Warn" { "Low-Medium" }
    default { "Medium" }
}
...
$record.deliberation.overrideModel.overrideTestSummary = if ($overrideApplied -and $overrideReason) { $overrideReason } else { "No override was applied because the consensus outcome remained acceptable under the configured rules." }
...
$record.deliberation.adjudication.ruleTriggered = $ruleTriggered
$record.deliberation.adjudication.humanEscalationConsidered = (@($humanEscalationReasons).Count -gt 0)
$record.deliberation.adjudication.residualRiskAfterMitigation = $residualRiskAfterMitigation
$record.deliberation.adjudication.decisionPath = $decisionPath
$record.deliberation.adjudication.uncertaintyDrivers = $uncertaintyDrivers
